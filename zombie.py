...
def import_carte(filename):
    '''
    Stocke les coordonnées d'un fichier .svg dans une liste.
    
    Args :
    filename : Il s'agit du nom du fichier .svg qui contient
               les coordonnées que l'on souhaite extraire.

    Returns : 
    france : Liste contenant des sous-listes de deux éléments 
             (x,y) pour les coordonnées de chaque points extraits
             du fichier filename.
    ...
    for i in xrange(0,len(coord_list),2):
        mapped.append([float(coord_list[i]), float(coord_list[i+1])])
    return mapped
...
#Vérifie le nombre d'agument nécessaire.
if len(sys.argv) != 3:
    sys.exit("Ce programme permet à partir d'un fichier de balisage (format.svg) d'afficher la propagation d'une pandémie Zombie.\n----ERREUR : Vous devez entrer un nom de fichier au format .svg ainsi que le Nombre de jour de simulation.\n")

#Vérifier que le fichier donné en argument existe
if not os.path.exists(sys.argv[1]):
    sys.exit ("Le fichier transmit en argument n'existe pas.\n")

    #Vérifier le format du fichier saisie en argument
if not re.search("[A-Za-z0-9]*\.svg",sys.argv[1]):
    sys.exit ("Le fichier transmit en argument n'est pas au bon format (format .svg demandé).\n")

#Vérifie que le nombre de jours de simulation a été transmit.
jours_simulation=re.compile('[0-9]+')
if not jours_simulation.search(sys.argv[2]):
    sys.exit("ERREUR : il faut indiquer en argument un nombre de jours")

mapped=import_carte(sys.argv[1])
print mapped
